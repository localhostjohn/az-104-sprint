...
import {listReviewState,recordQuestionAnswer,recordQuestionAnswers} from '@/lib/mistakes-db';

export async function GET(){
  try{return Response.json(await listReviewState());}
...
export async function POST(request:Request){
  try{
    ...
    if(Array.isArray(body.answers)){
      ...
      return Response.json(await recordQuestionAnswers(answers.map(item=>({question_id:Number(item.question_id),correct:item.correct as boolean}))));
    }
    ...
    return Response.json(await recordQuestionAnswer(questionId,correct));
